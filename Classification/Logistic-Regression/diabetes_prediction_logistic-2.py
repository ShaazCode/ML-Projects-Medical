...
# it’s just tagging high glucose cases with 1 and others with 0.

df["Insulin_Flag"] = np.where(df["Insulin"] > 0, 1, 0)
# It creates a yes/no flag to indicate whether insulin was measured/present (value > 0) or not, helping the model handle missing or zero insulin values easily.
...
# StandardScaler normalizes your data.
# It makes all features comparable so ML models don’t get confused by different ranges.
